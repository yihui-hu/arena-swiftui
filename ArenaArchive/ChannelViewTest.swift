        ...
        ScrollView {
            LazyVGrid(columns: columns, spacing: 10) {
                ForEach(channelData.contents ?? [], id: \.self.id) { block in
                    NavigationLink(destination: destinationViewTest(for: block)) {
...
@ViewBuilder
func destinationViewTest(for block: Block) -> some View {
    if block.baseClass == "Block" {
        BlockView(blockId: block.id)
    } else {
...
#Preview {
    ChannelViewTest(channelSlug: "maplestory")
}
